...
class FrameSideDecisionService:
	def __init__(self):
	...
	def decide_side(
		...
		if x_left < left_frame_limit:
			decision = FrameSideDecision(side=Side.LEFT, confidence=1.0)
		elif x_right > right_frame_limit:
			decision = FrameSideDecision(side=Side.RIGHT, confidence=1.0)
		else:
			...
			side = Side.RIGHT if combined >= 0 else Side.LEFT
			confidence = abs(combined)
			decision = FrameSideDecision(side=side, confidence=confidence)
